 ...
 * 
 */
public class ClipImageViews extends ImageView implements View.OnTouchListener,
        ViewTreeObserver.OnGlobalLayoutListener {

    private static final int BORDERDISTANCE = ClipViews.BORDERDISTANCE;

    public static final float DEFAULT_MAX_SCALE = 4.0f;
    public static final float DEFAULT_MID_SCALE = 2.0f;
    public static final float DEFAULT_MIN_SCALE = 1.0f;

    public static final float DEFAULT_MIN_SCALE_S = 0.85f;
    public static final float DEFAULT_MIN_SCALE_XS = 0.7f;

    private float minScaleXs = DEFAULT_MIN_SCALE_XS;
    ...
    public ClipImageViews(Context context) {
        this(context, null);
    }

    public ClipImageViews(Context context, AttributeSet attr) {
        this(context, attr, 0);
    }

    public ClipImageViews(Context context, AttributeSet attr, int defStyle) {
    ...
    private void configPosition() {
        ...
        if (drawableWidth <= drawableHeight) {
            ...
            if (drawableWidth < borderlength) {
                ...
                minScaleXs = (float) borderlength / drawableWidth;
            }
            // 图片宽度大于高度
        } else {
            if (drawableHeight < borderlength) {
                ...
                minScaleXs = (float) borderlength / drawableHeight;
            }
        }
        // 移动居中
    ...
    private class MultiGestureDetector extends
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            if (getDrawable() != null
                    && ((scale < maxScale && scaleFactor > minScaleXs) || (scale > minScaleXs && scaleFactor < minScaleXs))) {
                if (scaleFactor * scale < minScaleXs) {
                    scaleFactor = minScaleXs / scale;
                }
    ...
    private class AnimatedZoomRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (((deltaScale > 1f) && (currentScale < targetZoom))
                ...
                // again
                postOnAnimation(ClipImageViews.this, this);
    ...
    public Bitmap clip() {
        ...
        draw(canvas);
        Bitmap reBm = Bitmap.createBitmap(bitmap,
     ...
     * @return
     */
    public Bitmap clip2(Bitmap bitmap1,Bitmap bitmap2) {
    	Bitmap bitmap = Bitmap.createBitmap(getWidth(), getHeight(),
                Bitmap.Config.ARGB_4444);
        Canvas canvas = new Canvas(bitmap);
        draw(canvas);
        Paint paint = new Paint();
        paint.setColor(getResources().getColor(R.color.top));
