 ...
 * 找回密码
 */
public class GetCodeActivity extends BaseActivity implements OnFocusChangeListener, TextWatcher {
	private EditText getcode_edit_code, getcode_edit_code1;
	private ImageView getcode_code_edit_clear, getcode_code1_edit_clear;
		...
		public void handleMessage(Message msg) {
			...
			case 2:
				if (LoginUtil.login_handlered) {
					Log.d("345abc", "获取新token-找回密码");
					LoginUtil.login_handlered = false;
					mToken = sp.getString("mToken", null);
					if (mToken != null) {
						getInfo();
					} else {
						Toast.makeText(GetCodeActivity.this, "网络不佳，请稍后再试", Toast.LENGTH_SHORT).show();
					}
				} else {
			...
			case 3:
				...
				Builder bd6 = new MyAlertDialog.Builder(GetCodeActivity.this);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// getcode_edit_account = (EditText)
		// findViewById(R.id.getcode_edit_account);
		// getcode_edit_account.setOnFocusChangeListener(this);
		// getcode_edit_account.addTextChangedListener(this);
		// getcodesms_edit_ccaptcha = (EditText)
		// findViewById(R.id.getcodesms_edit_ccaptcha);
		...
		sp = GetCodeActivity.this.getSharedPreferences("huizhongjia", Context.MODE_PRIVATE);
		editor = sp.edit();
		...
		mc.start();
		dialog = ProgressDialogUtil.createLoadingDialog(GetCodeActivity.this, "修改中", true, false);
	...
	public void onClick(View v) {
		...
		case R.id.back_getcode:
			if (imm.isActive() && GetCodeActivity.this != null) {
				imm.hideSoftInputFromWindow(GetCodeActivity.this.getCurrentFocus().getWindowToken(),
	...
	private void getCode() {
		...
		// return;
		// }
		password = getcode_edit_code.getText().toString();
		if (StringUtil.isEmpty(password)) {
			Toast.makeText(GetCodeActivity.this, "请输入密码", Toast.LENGTH_SHORT).show();
			return;
		}
		if (password.trim().length() < 6) {
			Toast.makeText(GetCodeActivity.this, "密码最低为6位", Toast.LENGTH_SHORT).show();
			return;
		}
		if (StringUtil.isEmpty(getcode_edit_code1.getText().toString())) {
			Toast.makeText(GetCodeActivity.this, "请确认密码", Toast.LENGTH_SHORT).show();
			return;
		}
		if (!password.equals(getcode_edit_code1.getText().toString())) {
			Toast.makeText(GetCodeActivity.this, "两次输入密码不同，请重新输入", Toast.LENGTH_SHORT).show();
			return;
		}
	...
	private void getTokenLocal() {
		mToken = new TokenManager(GetCodeActivity.this).getToken();
		if (mToken != null) {
			getInfo();
		} else {
			LoginUtil.login_handlered = false;
	...
	private void getInfo() {
		try {
			...
			String app_nonce = StringUtil.getPhoneIMEI(GetCodeActivity.this);
			String data = Encryption.encryptd(Constant.code, Constant.code, password);

			String url_getcode = Constant.phpUrl + "/wap/api.php?action=FIND_PWD&app_nonce=" + app_nonce + "&mobile="
					+ account + "&tocken=" + mToken + "&data=" + data;
			Log.d("123456", "找回密码地址为" + url_getcode);
			dialog.show();
			NetHelper.get(url_getcode, new SimpleSingleBeanNetHandler<Register>(GetCodeActivity.this) {
				...
				protected void onSuccess(Register bean) {
					...
					try {
						...
						switch (result) {
						case 0:
							Toast.makeText(GetCodeActivity.this, "修改密码成功,请重新登录", Toast.LENGTH_SHORT).show();
							editor.remove("captcha_md5");
							editor.commit();
							if (GetCodeSmsActivity.gcs_instance != null) {
								GetCodeSmsActivity.gcs_instance.finish();
							}
							finish();
							break;

						case 99:
							// Toast.makeText(GetCodeActivity.this,
							// "密码修改",Toast.LENGTH_SHORT).show();
							// editor.remove("captcha_md5");
							// editor.commit();
							...
							break;
						default:
							Toast.makeText(GetCodeActivity.this, bean.info, Toast.LENGTH_SHORT).show();
							editor.remove("captcha_md5");
							editor.commit();
							break;
						}
						...
						// TODO: handle exception
						Toast.makeText(GetCodeActivity.this, "修改密码失败", Toast.LENGTH_SHORT).show();
					}
				...
				protected void onError(int errorCode, String errorMsg) {
					 ...
					 */
					dialog.hide();
					Toast.makeText(GetCodeActivity.this, "通信失败", Toast.LENGTH_SHORT).show();
				}
