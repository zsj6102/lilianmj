 ...
 *
 */
public class FragmentCart extends Fragment implements OnClickListener{
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		MainTabActivity.title_main.setText("购物车");
		sp = getActivity().getSharedPreferences("huizhongjia", Context.MODE_PRIVATE);
		editor = sp.edit();
//		if (rootView == null) {
			rootView = inflater.inflate(R.layout.activity_shoppingcar, null);
			initView(rootView);
	        getTokenLocal();
//		}
	...
	private void initView(View layout) {
		...
		loading = (LinearLayout) layout.findViewById(R.id.linear_load);
        tvNoData = (TextView) layout.findViewById(R.id.shoppingCard_tv_noData);
        loading.setVisibility(View.VISIBLE);

        checkAll = (CheckBox) layout.findViewById(R.id.shoppingCar_chkb_all);
        checkAll.setOnClickListener(this);
        tvSum = (TextView) layout.findViewById(R.id.shoppingCar_tv_sum_money);
        lvContent = (ListView) layout.findViewById(R.id.shoppingCar_lv_content);
        layout.findViewById(R.id.shoppingCar_btn_balance).setOnClickListener(this);
        tvSum.setText(String.format(
                this.getResources().getString(R.string.money_describe), "0.00"));
        adapter = new CommonAdapter<Cart>(getActivity(), carts,
			...
			public void convert(ViewHolder viewHolder, Cart cart) {
		        ...
		        if(!TextUtils.isEmpty(cart.getOne_price())){
		        	tvNewPrice.setText(String.format(
		        			getActivity().getResources().getString(R.string.money_describe),
		        	...
		        	tvPerMoney.setText(String.format(getActivity().getResources().getString(R.string.money_describe),
    ...
    public void onClick(View v) {
        ...
        case R.id.back_search_logininput:
			getActivity().finish();
			getActivity().overridePendingTransition(R.anim.anim_slider_left_in,
    ...
    private void settlement() {
        ...
        if (settlementNum < 1) {
            ToastUtil.getInstance().showToast(getActivity(), R.string.please_goods);
            return;
        }
 ...
 // 获取tocken
    ...
    private void iniData() {
		...
		NetHelper.post(Constant.GETCART, params,
				new SimpleSingleBeanNetHandler<Captcha>(getActivity()) {
					...
					protected void onSuccess(Captcha bean) {
						...
						if (bean.result.equals("0")) {
							...
							Toast.makeText(getActivity(), bean.message + "",
        ...
        public void onClick(View v) {
            ...
            case R.id.shoppingCard_ll_down:
                ...
                if (number > 1) {
                    ...
                    final Dialog dialog = LoadingDialogShow.loading(getActivity());
 ...
 // 删除提示
    private void deleteDialog(final String cartItemId, final int position) {
    	final MyDialog.MyBuilder dailog = new MyDialog.MyBuilder(
				getActivity());
