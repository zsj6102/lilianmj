...
public class SinaWeiboAPI {
	 ...
	 * params.add("access_token", accessToken); //*AsyncWeiboRunner.request(url,
	 * params, httpMethod, listener); }
	 */
	/**
	 * 根据用户ID获取用户信息
	 * 
	 ...
	 * RequestListener listener) { WeiboParameters params = new
	 * WeiboParameters(); params.add("status", content); if
	 * (!TextUtils.isEmpty(lon)) { params.add("long", lon); } if
	 * (!TextUtils.isEmpty(lat)) { params.add("lat", lat); }
	 ...
	 * RequestListener listener) { WeiboParameters params = new
	 * WeiboParameters(); params.add("status", content); params.add("pic",
	 * file); if (!TextUtils.isEmpty(lon)) { params.add("long", lon); } if
	 * (!TextUtils.isEmpty(lat)) { params.add("lat", lat); }
	 ...
	 */
	/*
	 * public void endSession(RequestListener listener) { WeiboParameters params
	 * = new WeiboParameters(); request(URL_ACCOUNT + "/end_session.json",
